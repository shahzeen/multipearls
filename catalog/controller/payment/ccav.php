...
class ControllerPaymentCcav extends Controller {
	public function index() {
		...
		$this->load->model("payment/ccav");
		$this->load->model('checkout/order');            
        $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
		$redirect = $this->model_payment_ccav->proceed($order_info);			
		$data['continue'] = $redirect;
		return $this->load->view('default/template/payment/ccav.tpl', $data);
	}
	...
	public function response()
	{
		...
		$this->load->model('payment/ccav');
		$order_status = $this->model_payment_ccav->response($encResponse);
