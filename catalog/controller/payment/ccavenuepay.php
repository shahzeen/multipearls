...
$file = DOM_APG_PATH_PG_MAIN_201 . "ccavenue_main.php";
if (file_exists($file))
{
	include_once ($file);
...
class ControllerPaymentCcavenuepay extends Controller{
	public function index(){
		...
		if ($order_info)
		{
			...
			$Url = $this->getBaseUrl('payment/ccavenuepay/callback');
			...
			$redirect_url = $this->getBaseUrl('payment/ccavenuepay/callback');
			$cancel_url = $this->getBaseUrl('payment/ccavenuepay/callback');
			...
			$test_mode = $this->config->get('ccavenuepay_test_mode');
			$data['action'] = $cbdom->getPaymentGatewayUrl($test_mode);
			...
			$form_response = $cbdom->getfrontform('', json_encode($passdata));
			...
			if ($order_status == "Success")
			{
				...
				if ($response_array['order_id'] == $sessionOrderInfo['order_id'] && $response_array['amount'] == $sessionOrderInfo['total'])
				{
					...
					if ($payment_confirmation_mail === 1)
					{
						...
						$payment_confirmation_mail_body = sprintf($mail_body, $customer_name, $order_id, $order_info['date_added'], $order_info['payment_method'], $order_info['shipping_method'], $this->currency->format($order_info['total'], $this->session->data['currency']));
						$to = $order_info['email'];
						$mail = new Mail();
						$mail->setTo($to);
