	...
	public function index($setting) {
		...
		if ($this->customer->isLogged()) {
			...
			if(isset($this->request->cookie['recently_viewed']) && !empty($this->request->cookie['recently_viewed'])) {
				...
				foreach($recently_viewed as $k=>$v){
					$this->model_module_recently_viewed->setRecentlyViewedProducts($this->customer->getId(), $k, $v);
				}
				unset($this->request->cookie['recently_viewed']);
				setcookie('recently_viewed', '', 0, '/', $this->request->server['HTTP_HOST']);
			}
			
			if($product_ids = $this->model_module_recently_viewed->getRecentlyViewedProducts($this->customer->getId(), $setting['limit'], $current_product_id)){
		...
		if ($results) {
			...
			if(version_compare(VERSION, '2.2.0.0', '>=')) {
				...
				return $this->load->view('module/recently_viewed', $data);
